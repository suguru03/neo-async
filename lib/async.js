(function (global, factory) {
   ...
   */
  var each = createEach(
    createArrayEach(onlyOnce),
    createBaseEach(onlyOnce),
    createSymbolEach(onlyOnce)
  );
  ...
  var concat = createConcat(
    createArrayEach(onlyOnceWithArgs),
    createBaseEach(onlyOnceWithArgs),
    createSymbolEach(onlyOnceWithArgs)
  );
   ...
   * @private
   */
  function createArrayEach(onlyOnce) {

    return function arrayEach(array, iterator, callback) {
      var index = -1;
      var size = array.length;
   ...
   * @private
   */
  function createBaseEach(onlyOnce) {

    return function baseEach(object, iterator, callback, keys) {
      var key;
      var index = -1;
      var size = keys.length;
   ...
   * @private
   */
  function createSymbolEach(onlyOnce) {

    return function symbolEach(collection, iterator, callback) {
   ...
   * @param {Function} func
   */
  function onlyOnceWithArgs(func) {
    return function(err, res) {
      var fn = func;
      func = throwError;
      switch (arguments.length) {
        ...
        case 2:
          return fn(err, res);
        default:
          return fn(err, slice(arguments, 1));
      }
    };
  }
  ...
  function concatLimit(collection, limit, iterator, callback) {
    ...
    function arrayIterator() {
      if (started < size) {
        iterator(collection[started++], onlyOnceWithArgs(done));
      }
    }
    ...
    function arrayIteratorWithIndex() {
      index = started++;
      if (index < size) {
        iterator(collection[index], index, onlyOnceWithArgs(done));
      }
    }

    function symbolIterator() {
      if ((item = iter.next()).done === false) {
        iterator(item.value, onlyOnceWithArgs(done));
      }
    }

    function symbolIteratorWithKey() {
      if ((item = iter.next()).done === false) {
        iterator(item.value, started++, onlyOnceWithArgs(done));
      }
    }

    function objectIterator() {
      if (started < size) {
        iterator(collection[keys[started++]], onlyOnceWithArgs(done));
      }
    }
    ...
    function objectIteratorWithKey() {
      if (started < size) {
        key = keys[started++];
        iterator(collection[key], key, onlyOnceWithArgs(done));
      }
    }
