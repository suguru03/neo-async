(function() {
  ...
  EventEmitter.prototype.on = function on(key, callback) {
    ...
    if (typeof key === 'object') {
      ...
      if (Array.isArray(callback)) {
        _arrayEach(callback, function(func) {
          self._events[key].push(createCallback(func));
        });
      } else {
        self._events[key].push(createCallback(callback));
      }
    }
    return self;
  ...
  EventEmitter.prototype.once = function once(key, callback) {
    ...
    if (typeof key === 'object') {
      ...
      if (Array.isArray(callback)) {
        _arrayEach(callback, function(func, index) {
          callback[index] = createOnce(func);
        });
      } else {
        callback = createOnce(callback);
      }
