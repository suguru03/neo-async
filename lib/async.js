(function() {
  ...
  var each = createEach(arrayEach, objectEach);
   ...
   * @private
   */
  function objectEach(object, iterator, callback, keys) {
   ...
   *
   */
  function createEach(arrayEach, objectEach) {
    ...
    return function each(collection, iterator, callback, thisArg) {
      ...
      if (Array.isArray(collection)) {
        ...
        var keys = Object.keys(collection);
        size = keys.length;
        objectEach(collection, _iterator, done, keys);
      }
      if (!size) {
        callback();
      }
