(function() {
   ...
   * @namespace applyEach
   */
  var applyEach = createApplyEach(each);

  /**
   * @memberof async
   * @namespace applyEachSeries
   */
  var applyEachSeries = createApplyEach(eachSeries);
  ...
  } else if (root && root.async) {
    root.neo_async = exports;
  } else {
    root.async = exports;
  }
   ...
   * @param {Function} iterator - The function invoked per iteration.
   */
  function arrayEachSyncRight(array, iterator) {
    var size = array.length;
  ...
  function baseQueue(isQueue, worker, concurrency, payload) {
    ...
    function _next(q, tasks) {
      ...
      return function done(err, res) {
        ...
        if (arguments.length <= 2) {
          arrayEachSync(tasks, function(task) {
    ...
    function resume() {
      ...
      q.paused = false;
      timesSync(q.concurrency, function() {
        nextTick(q.process);
      });
    }
  }
  ...
  function autoInject(tasks, concurrency, callback) {
    ...
    auto(newTasks, concurrency, done);

    function iterator(task, key) {
      var params, taskLength;
      if (Array.isArray(task)) {
    ...
    function done(err, results) {
      ...
      while (size) {
        result[size--] = results[params[size]];
      }
      result[0] = err;
  ...
  function retry(times, task, callback) {
    ...
    if (arguments.length < 3 && typeof times === 'function') {
      ...
      switch (typeof times) {
        case 'object':
          interval = +times.interval || DEFAULT_INTERVAL;
    ...
    function intervalCallback(err, res) {
      ...
      setTimeout(intervalIterator, interval);
    }
  }
  ...
  function timeout(func, millisec, info) {
    ...
    function timeoutCallback() {
      ...
      var err = new Error('Callback function "' + name + '" time out.');
      err.code = 'ETIMEDOUT';
      if (info) {
        err.info = info;
      }
  ...
  EventEmitter.prototype.getListeners = function(key) {
    ...
    var allTasks = {};
    baseEachSync(events, function(_events, key) {
    ...
    function getTasks(events) {
      ...
      arrayEachSync(events, function(task, index) {
  ...
  EventEmitter.prototype.addListener = function addListener(key, functions) {
    ...
    if (typeof key === 'object') {
      baseEachSync(key, function(func, key) {
    ...
    if (Array.isArray(functions)) {
      arrayEachSync(functions, function(func) {
  ...
  EventEmitter.prototype.addOnceListener = function addOnceListener(key, functions) {
    ...
    if (Array.isArray(functions)) {
      arrayEachSync(functions, function(func, index) {
  ...
  EventEmitter.prototype.removeListener = function removeListener(key, functions) {
    ...
    if (Array.isArray(functions)) {
      arrayEachSyncRight(functions, function(func) {
        removeListener.call(self, func, key);
      });
      return self;
    }

    arrayEachSyncRight(events, function(event, index) {
  ...
  EventEmitter.prototype.removeEvent = function removeEvent(key) {
    var self = this;
    if (!key) {
      baseEachSync(self._events, function(events, key) {
  ...
  EventEmitter.prototype.emit = function(key, callback, thisArg) {
    ...
    function done(err, res) {
      if (err) {
        ...
        arrayEachSyncRight(events, function(func, index) {
          if (func.ONCE === ONCE) {
            events.splice(index, 1);
          }
        });
