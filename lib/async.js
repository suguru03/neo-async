(function() {
  ...
  var _nextTick, asyncNextTick, asyncSetImmediate;
  createImmediate();
  ...
  function createImmediate() {
    ...
    if (typeof process === 'object' && typeof process.nextTick === 'function') {
      _nextTick = /^v0.10/.test(process.version) ? asyncSetImmediate : process.nextTick;
      asyncNextTick = /^v0/.test(process.version) ? asyncSetImmediate : process.nextTick;
    } else {
      asyncNextTick = _nextTick = asyncSetImmediate;
    }
  }
    ...
    return function transform(collection, accumulator, iterator, callback) {
      ...
      function done(err, bool) {
        if (err) {
          ...
          callback(null, Array.isArray(result) ? _arrayClone(result) : _objectClone(result));
          callback = noop;
        }
      }
    };
      ...
      function createCallback(value) {
        ...
        return function done(err, criteria) {
          ...
          if (err) {
            ...
            callback(null, _pluck(result, 'value'));
            callback = throwError;
          }
        };
      }
  ...
  function _queue(worker, concurrency, payload) {
    ...
    function _next(q, tasks) {
      ...
      return function done(err, res) {
        ...
        while (++index < size) {
          if (workersList[index] === firstTask) {
  ...
  function queue(worker, concurrency) {
    return _queue(_worker, concurrency, 1);
    function _worker(data, callback) {
      worker(data[0], callback);
    }
  ...
  function priorityQueue(worker, concurrency) {
    var q = queue(worker, concurrency, 1);
    q.push = push;
    delete q.unshift;
    return q;
  ...
  function auto(tasks, concurrency, callback) {
    ...
    function taskComplete(key) {
      ...
      _arrayEach(taskListeners, function(task) {
        task();
      });
      proceedQueue();
    }
