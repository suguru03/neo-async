(function() {
  ...
  function createImmediate() {
    ...
    if (typeof process === 'object' && typeof process.nextTick === 'function') {
      _nextTick = /^v0.10/.test(process.version) ? _setImmediate : process.nextTick;
    } else {
  ...
  function autoInject(tasks, concurrency, callback) {
    ...
    function iterator(task, key) {
      ...
      if (Array.isArray(task)) {
        ...
        index = 0;
        taskLength = task.length;
        params = Array(taskLength);
      }
      var args = parseParams(task);
      var argIndex = -1;
      while (++argIndex < taskLength - 1) {
        params[index++] = args[argIndex];
      }
      params[index] = newTask;
      newTasks[key] = params;
      ...
      function newTask(results, done) {
        switch (taskLength) {
          case 1:
            ...
            break;
          case 2:
            task(results[args[0]], done);
            break;
          case 3:
            task(results[args[0]], results[args[1]], done);
            break;
          case 4:
            task(results[args[0]], results[args[1]], results[args[2]], done);
            break;
          default:
            var i = -1;
            while(++i < taskLength - 1) {
              args[i] = results[args[i]];
            }
            args[i] = done;
