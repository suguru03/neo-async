(function() {
   ...
   *
   * @private
   * @param {Object} collection - The collection to convert.
   */
  ...
  function pick(collection, iterator, callback, thisArg, opposite) {
    ...
    function createPickCallback(key, value) {
      ...
      return function done(res) {
        ...
        if (--size === 0) {
          callback(isArray ? _compact(result) : result);
        }
      };
    }
    ...
    function createPickCallbackWithError(key, value) {
      ...
      return function done(err, res) {
        ...
        if (err) {
          callback(err, isArray ? _compact(result) : result);
          callback = noop;
          return;
        }
        if (!!res === bool) {
        ...
        if (--size === 0) {
          callback(undefined, isArray ? _compact(result) : result);
        }
      };
    }
  }
  ...
  function pickSeries(collection, iterator, callback, thisArg, opposite) {
    callback = callback || noop;
    var size, key, value, keys, iterate, result, called, done;
    var bool = !opposite;
    var completed = 0;
    var resultCount = -1;
    var enableError = callback.length === 2;
    ...
    if (Array.isArray(collection)) {
      ...
      result = [];
      done = enableError ? arrayCallbackWithError : arrayCallback;
      iterate = iterator.length === 3 ? arrayIteratorWithIndex : arrayIterator;
    } else if (collection && typeof collection === 'object') {
      keys = Object.keys(collection);
      size = keys.length;
      ...
      result = {};
      done = enableError ? objectCallbackWithError : objectCallback;
      iterate = iterator.length === 3 ? objectIteratorWithKey : objectIterator;
    } else {
      return enableError ? callback(undefined, []) : callback([]);
    }
    ...
    function arrayCallbackWithError(err, res) {
      if (called) {
        throw new Error('Callback was already called.');
      }
      called = true;
    ...
    function objectCallbackWithError(err, res) {
      if (called) {
        throw new Error('Callback was already called.');
      }
      called = true;
